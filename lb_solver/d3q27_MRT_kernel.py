...
class ModelConfig:
    ...
    def col_stream_core(self, lbm: ti.template(), f_pre: ti.template(), f_post: ti.template()):
        for i, j, k in ti.ndrange((ti.static(self.pop), ti.static(lbm.nd[0] - self.pop)), (ti.static(self.pop), ti.static(lbm.nd[1] - self.pop)), (ti.static(self.pop), ti.static(lbm.nd[2] - self.pop))):
            ...
            m021_eq = x19 + x5
            m202_eq = x20 + x21 + x24
            m211_eq = v*x8 + x4 * self.INV_3
            m220_eq = x16*x7 + x24 + x25
            m112_eq = v*x15 + x2 * self.INV_3
            m121_eq = w*x18 + x1 * self.INV_3
            m022_eq = x13*x17 + x20 + x22 + x25
            m212_eq = v*x22 + x11*x13 + x14*x9 + x7*x9
            m221_eq = w*x22 + x16*x8 + x19 * self.INV_3 + x8 * self.INV_3
            m122_eq = u*x22 + x13*x18 + x15 * self.INV_3 + x18 * self.INV_3
            m222_eq = rho * self.INV_27 + x13*x22 + x13*x23 + x13*x25 + x16*x21 + x16*x22 + x16*x23 + x22*x6
            mxx_eq = -x17 + x7
            mzz_eq = -x14 + x7
            mP_eq = rho + x14 + x17 + x7
